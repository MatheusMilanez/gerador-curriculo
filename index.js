...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document
    ...
    .addEventListener("submit", function (event) {
      ...
      if (!isValidDate(graduationDate)) {
        alert(
          "Por favor, insira uma data de conclusão válida no formato DD/MM/YYYY."
        );
        return;
      }
      if (!isValidDate(startDate)) {
        alert(
          "Por favor, insira uma data de início válida no formato DD/MM/YYYY."
        );
        return;
      }
      if (endDate && !isValidDate(endDate)) {
        alert(
          "Por favor, insira uma data de término válida no formato DD/MM/YYYY."
        );
        return;
      }
  ...
  document
    .getElementById("completed-yes")
    .addEventListener("click", function () {
      document.getElementById("graduation-date-container").style.display =
        "block"; // Show graduation date input
    });

  document
    .getElementById("completed-no")
    .addEventListener("click", function () {
      document.getElementById("graduation-date-container").style.display =
        "none"; // Hide graduation date input
  ...
  function addCurrentQuestionListeners(index) {
    document
      .getElementById(`current-yes-${index}`)
      .addEventListener("click", function () {
        ...
        document
          .getElementById("work-experience-container")
          .appendChild(ongoingText);
      });

    document
      .getElementById(`current-no-${index}`)
      .addEventListener("click", function () {
        document.getElementById(`end-date-container-${index}`).style.display =
          "block"; // Show end date input
      });
  }
  ...
  document
    ...
    .addEventListener("submit", function (event) {
      ...
      doc.text(
        ...
        10,
        60 + educationEntries.length * 10 + 10
      );
      for (let i = 0; i < workEntries.length; i++) {
        doc.text(
          `- ${workEntries[i].value}`,
          10,
          70 + educationEntries.length * 10 + i * 10
        );
      }
